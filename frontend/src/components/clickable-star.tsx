...
export default function ClickableStar({ initialState = false, size = 24, onChange, className, symbol }: ClickableStarProps) {
    console.log('ClickableStar props:', { initialState, size, symbol, className });
    
    // Early return if symbol is invalid
    if (!symbol || symbol.trim() === '') {
        console.error('ClickableStar: Invalid symbol provided:', symbol);
        return (
            <Button variant="ghost" size="icon" disabled className="p-0 h-auto w-auto opacity-50">
    ...
    try {
        ...
        console.log(`Watchlist context not available for ${symbol}:`, error instanceof Error ? error.message : String(error))
    }
    ...
    const handleClick = () => {
        ...
        if (watchlist) {
            console.log(`Updating watchlist: ${newState ? 'adding' : 'removing'} ${symbol}`)
            if (newState) {
