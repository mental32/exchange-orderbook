/*
 * Clerk Backend API
 *
 * The Clerk REST Backend API, meant to be accessed by backend servers.  ### Versions  When the API changes in a way that isn't compatible with older versions, a new version is released. Each version is identified by its release date, e.g. `2025-04-10`. For more information, please see [Clerk API Versions](https://clerk.com/docs/versioning/available-versions).  Please see https://clerk.com/docs for more information.
 *
 * The version of the OpenAPI document: 2025-04-10
 * Contact: support@clerk.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::Deserialize;
use serde::Serialize;

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct CreateMachineRequest {
    /// The name of the machine
    #[serde(rename = "name")]
    pub name: String,
    /// Array of machine IDs that this machine will have access to. Maximum of 25 scopes per machine.
    #[serde(rename = "scoped_machines", skip_serializing_if = "Option::is_none")]
    pub scoped_machines: Option<Vec<String>>,
    /// The default time-to-live (TTL) in seconds for tokens created by this machine. Must be at least 1 second.
    #[serde(rename = "default_token_ttl", skip_serializing_if = "Option::is_none")]
    pub default_token_ttl: Option<i32>,
}

impl CreateMachineRequest {
    pub fn new(name: String) -> CreateMachineRequest {
        CreateMachineRequest {
            name,
            scoped_machines: None,
            default_token_ttl: None,
        }
    }
}
