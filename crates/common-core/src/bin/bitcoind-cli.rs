        ...
        .block_on(async move {
            ...
            let st = match command {
                ...
                Command::GetBlock { blockhash } => {
                    let t = client
                        .get_block_info(&blockhash.parse().unwrap())
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                ...
                Command::GetBlockFilter { blockhash } => {
                    let t = client
                        .get_block_filter(&blockhash.parse().unwrap())
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                ...
                Command::SetLabel { address, label } => {
                    ...
                    .unwrap();
                    let t = client
                        .set_label(&rpc::Address(address), &label)
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                ...
                Command::GetAddressInfo { address } => {
                    ...
                    .unwrap();
                    let t = client
                        .get_address_info(&rpc::Address(address))
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                ...
                Command::InvalidateBlock { blockhash } => {
                    let t = client
                        .invalidate_block(&blockhash.parse().unwrap())
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                Command::ReconsiderBlock { blockhash } => {
                    let t = client
                        .reconsider_block(&blockhash.parse().unwrap())
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                ...
                Command::GetMempoolEntry { txid } => {
                    let t = client
                        .get_mempool_entry(&txid.parse().unwrap())
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                ...
                Command::SendRawTransaction { hexstring } => {
                    let t = client
                        .send_raw_transaction(hexstring.as_bytes())
                        .await
                        .unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
                ...
                Command::ScanTxOutset => {
                    let t = client.scan_tx_out_set_blocking(&[]).await.unwrap();
                    serde_json::to_string_pretty(&t).unwrap()
                }
            };
